...
DEFAULT_DEST_DIR = Path("install") / "python"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Prepare embedded Python for release packages")
    parser.add_argument("--target-os", choices=["win", "macos", "linux"], default=None)
    parser.add_argument("--target-arch", default=None)
    parser.add_argument("--dest-dir", default=None, help="输出目录（默认 install/python）")
    ...
    dest_dir = Path(args.dest_dir) if args.dest_dir else DEFAULT_DEST_DIR
